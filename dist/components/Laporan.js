                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 text-gray-500 text-xs uppercase tracking-wide">
                            <tr>
                                ...
                                <th class="text-left px-5 py-3">Rating & Feedback</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr v-for="item in filteredData" :key="item.id"
                                ...
                                <td class="px-5 py-4">
                                    <span class="font-mono text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded block mb-1 text-center font-bold">
                                        {{ item.kode_laporan }}
                                    </span>
                                    <span v-if="item.is_banding === '1' || item.is_banding === 1"
                                        class="text-[10px] font-bold bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded uppercase block text-center shadow-sm">
                                        Banding
                                    </span>
                                </td>
                                ...
                                <td class="px-5 py-4 text-xs">
                                    <div v-if="item.rating">
                                        ...
                                        <p class="text-gray-400 truncate max-w-[150px]" :title="item.feedback_warga">{{ item.feedback_warga }}</p>
                                    </div>
                                    <span v-else class="text-gray-300 italic">Belum diulas</span>
                                </td>
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div v-if="form.rating" class="col-span-2 bg-slate-50 p-4 rounded-xl border border-slate-100/80 mt-2">
                            <h4 class="text-xs uppercase tracking-wider font-bold text-slate-500 mb-1">💚 Ulasan & Rating Warga</h4>
                            <div class="flex items-center space-x-1 text-yellow-500 text-sm mb-1">
                                <span v-for="star in parseInt(form.rating)" :key="star">★</span>
                                <span v-for="star in (5 - parseInt(form.rating))" :key="'empty-'+star" class="text-gray-250">★</span>
                            </div>
                            <p class="text-sm text-slate-700 italic font-medium">"{{ form.feedback_warga }}"</p>
                        </div>

                        <!-- Banding / Urgensi Warga (Jika Ada) -->
                        <div v-if="form.is_banding === '1' || form.is_banding === 1" class="col-span-2 bg-amber-50 p-4 rounded-xl border border-amber-150 mt-2">
                            <h4 class="text-xs uppercase tracking-wider font-bold text-amber-900 mb-1">📢 Alasan Urgensi Banding Warga</h4>
                            <p class="text-sm text-slate-755 leading-relaxed whitespace-pre-line font-medium">{{ form.alasan_urgensi }}</p>
                        </div>
                    </div>
                </div>
    ...
    methods: {
        defaultForm() {
            return {
                ...
                rating: null, feedback_warga: '',
                is_banding: 0, alasan_urgensi: '',
            };
        },
        ...
        openModal(item = null) {
            ...
            if (item) {
                ...
                this.form   = {
                    ...
                    feedback_warga:   item.feedback_warga || '',
                    is_banding:       item.is_banding || 0,
                    alasan_urgensi:   item.alasan_urgensi || '',
                };
                this.editId = item.id;
            } else {
                this.isEdit = false;
